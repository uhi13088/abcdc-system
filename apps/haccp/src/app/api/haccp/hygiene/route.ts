...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: userProfile } = await adminClient
      .from('users')
      .select('id, company_id, name, store_id, current_store_id, current_haccp_store_id')
      .eq('auth_id', userData.user.id)
      .single();
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const { data: userProfile } = await adminClient
      .from('users')
      .select('id, company_id, name, role, store_id, current_store_id, current_haccp_store_id')
      .eq('auth_id', userData.user.id)
      .single();
